...
import { UploadCloud, Image as ImageIcon, CheckCircle, AlertTriangle, XCircle, Info, RefreshCw, Activity, Search, Leaf, ShieldAlert } from 'lucide-react'
  ...
  return (
    <div className="space-y-6 w-full max-w-7xl mx-auto">
      <div className="flex items-center justify-between">
        ...
        <div className="text-xs bg-forest-100 dark:bg-forest-900/40 text-forest-600 dark:text-forest-300 px-3 py-1.5 rounded-full font-medium">
          Supported: Tomato
        </div>
      </div>
                  ...
                  <div className="flex items-center justify-between mb-1">
                    <h2 className="text-xl font-bold uppercase tracking-wide">
                      {result.prediction.status === 'healthy' ? 'Healthy Leaf' : 
                       result.prediction.status === 'uncertain' ? 'Uncertain / Unsupported' : 
                       'Disease Detected'}
                    </h2>
                    <span className="text-sm font-bold bg-white/50 dark:bg-black/30 px-2 py-1 rounded">
                      {(result.prediction.confidence * 100).toFixed(1)}% Conf
                    </span>
                  </div>
          ...
          <div className="agri-card">
            ...
            {loadingHistory ? (
              ...
              <div className="space-y-4">
                ...
                {analytics.most_frequent_disease !== 'None' && (
                  ...
                  <div key={scan.id} className="flex gap-3 items-center p-2 rounded-lg hover:bg-forest-50 dark:hover:bg-forest-900/40 transition-colors border border-transparent hover:border-forest-100 dark:hover:border-forest-800">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className={`text-sm font-semibold truncate ${
                        ...
                        {scan.prediction_status === 'diseased' ? scan.disease_name : scan.prediction_status.charAt(0).toUpperCase() + scan.prediction_status.slice(1)}
                      </div>
...
// Add ScanLine icon manually since it wasn't imported from lucide-react in the original list
import { ScanLine } from 'lucide-react'
export default DiseaseDetection
"""
with open('frontend/src/components/DiseaseDetection.jsx', 'w', encoding='utf-8') as f:
    f.write(disease_jsx)
print("DiseaseDetection.jsx updated.")
